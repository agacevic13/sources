...
static int ble_gap_event (struct ble_gap_event *event, void *arg)
{
    ...
    case BLE_GAP_EVENT_DISC:
        ...
        printf("discovered device %.*s, len: %d, %d\n", fields.name_len, fields.name, fields.name_len, strlen(DEVICE_NAME));
        if((fields.name_len == strlen(DEVICE_NAME)) && (!memcmp(fields.name, DEVICE_NAME, strlen(DEVICE_NAME))))
        {
            printf("device found\n");
            ble_gap_disc_cancel();
